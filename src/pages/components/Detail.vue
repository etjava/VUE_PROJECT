...
export default {
  ...
  methods:{
    getImageUrl(image){
      return serverUrl('image/'+image);
    },
    imageClick(){
      this.$refs.file.click();
    },
    showGalleryClick(){
      this.galleryImage=serverUrl('image/'+this.phoneBook.image);
    ...
    delInfo(){
      if(confirm("确定要删除吗？")){
        let url=serverUrl('phoneBook/delete');
        let token=window.localStorage.getItem('token')
        axios.defaults.headers.common['token'] = token;
        axios.get(url,{
          params:{"id":this.phoneBook.id}
        }).then(response=>{
    ...
    updateInfo(){
      let url=serverUrl('phoneBook/save');
      let token=window.localStorage.getItem('token')
      axios.defaults.headers.common['token'] = token;
    ...
    // 根据ID查询电话簿信息
    getInfo(){
      let url=serverUrl('phoneBook/findById');
      let token=window.localStorage.getItem('token')
      axios.defaults.headers.common['token'] = token;
      axios.get(url,{
        params:{
